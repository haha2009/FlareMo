...
公开分享路径是唯一例外。`/share/*` 和 `/api/public/shares/*` 用于
public share，生产 Access application 必须给这两类路径配置明确的
`bypass` policy。这个 bypass 只跳过 Cloudflare Access 登录，不跳过
FlareMo 的 share token 校验。公开分享内容由 share token、过期时间和
memo 状态控制；归档、回收站、删除或过期的分享不会公开返回。
