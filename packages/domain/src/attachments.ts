...
export async function listAttachments(db: FlareMoDb, user: UserRow, input: ListAttachmentsInput = {}) {
  const filters = [eq(attachments.userId, user.id), isNull(attachments.deletedAt)];
  if (input.memoId) {
    filters.push(eq(attachments.memoId, parseResourceName(input.memoId, "memos")));
  }
...
export async function listMemoAttachments(db: FlareMoDb, user: UserRow, memoId: string) {
...
export async function getAttachmentById(db: FlareMoDb, user: UserRow, id: string) {
...
export async function bindMemoAttachments(db: FlareMoDb, user: UserRow, memoId: string, attachmentNames: string[]) {
  const normalizedMemoId = parseResourceName(memoId, "memos");
  await getMemoById(db, user, normalizedMemoId);
  const ids = attachmentNames.map((name) => parseResourceName(name, "attachments"));
  const now = new Date().toISOString();
  ...
  if (ids.length > 0) {
    const existing = await db
      .select()
      .from(attachments)
      .where(and(eq(attachments.userId, user.id), inArray(attachments.id, ids), isNull(attachments.deletedAt)));
  ...
  await db
    ...
    .where(and(eq(attachments.userId, user.id), eq(attachments.memoId, normalizedMemoId)));

  if (ids.length > 0) {
    await db
      .update(attachments)
      .set({ memoId: normalizedMemoId, updatedAt: now })
      .where(and(eq(attachments.userId, user.id), inArray(attachments.id, ids)));
  }

  return listMemoAttachments(db, user, normalizedMemoId);
}

export async function softDeleteAttachment(db: FlareMoDb, user: UserRow, id: string) {
